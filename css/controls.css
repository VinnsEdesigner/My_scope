    ...
    startLoop() {
        ...
        const tick = () => {
            ...
            if (State.simMode) {
                ...
                if (frame % 8 === 0) {
                    const vch = State.oscViewChannel || 'ch1';
                    const cbuf = vch === 'ch2' && State.ch2Data ? State.ch2Data
                               : vch === 'ch3' && State.ch3Data ? State.ch3Data
                               : State.dataArray;
                    const cl = dspClassify(cbuf, State.simSampleRate);
