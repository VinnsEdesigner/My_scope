...
const App = {
    ...
    // ── Worker health flag — set true if importScripts fails ──
    _workerFailed: false,
    ...
    _initWorker() {
        try {
            ...
            this.worker.onerror   = (err) => {
                ...
                this._workerFailed = true;  // ← flag so render loop uses fallbacks
                ...
                banner.innerText = '⚠️ Worker failed — using fallback mode';
    ...
    _post(payload, transfers) {
        if (!this.worker || this._workerFailed) return Promise.resolve({});
    ...
    initMicStream() {
        if (!State.micSource || !State.audioCtx) {
            console.warn('initMicStream: micSource not ready');
            return;
        }
        ...
        try {
            ...
            console.log('✓ CH1 mic analyser connected');
        } catch (err) {
    ...
    startLoop() {
        const tick = () => {
            ...
            if (State.simMode) {
                ...
                // ── Measurements: worker or fallback ──
                if (State.dataArray) {
                    if (!this._workerFailed) {
                        ...
                        Measurements.update(); // fallback when worker is dead
                    }
                }

                // ── Classify: worker or fallback ──
                ...
                if (State.dataArray && this._hasArtifacts() && !this._workerFailed) {
                    this._post({
                        cmd:        'artifacts',
                        dataArray:  new Uint8Array(State.dataArray),
                        params:     { ...Artifacts.params },
                        sampleRate: State.simSampleRate,  // ← FIXED: was missing
                    });
                }
    ...
    _handleCH2() {
        ...
        if (State.ch2Source === 'synth') {
            if (!this._workerFailed) {
                ...
                // Fallback — main-thread Simulator when worker is dead
                Simulator.generate(State.ch2Data, State.freqArray);
            }
        } else if (State.ch2Source === 'file' && AudioLoader.hasFile) {
            AudioLoader.fillFromFile(State.ch2Data);
        } else {
            // No source ready — generate silence (128 = 0V)
