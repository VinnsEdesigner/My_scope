...
const UI = {
    ...
    _enterSimMode() {
        ...
        // Show OSC pills on non-SIM tabs — style.display overrides the inline style in HTML
        const oscPills = document.getElementById('oscPills');
    ...
    updateAnalyserDisplay(result) {
        ...
        detected.forEach(item => {
            ...
            // barValue tracks the actual measurement magnitude; confidence is the certainty %
            if (barEl) barEl.style.width = (item.barValue ?? item.confidence) + '%';
            if (pctEl) pctEl.innerText   = item.confidence + '%';
        });

        // Reset bars for items NOT in detected (clear stale values)
