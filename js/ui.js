...
const UI = {
    ...
    switchTab(tab) {
        ...
        App.resize();
        Persist.save(); // immediate — tab switch is high priority
    },
    ...
    async onCH1FileSelected(input) {
        ...
        await AudioLoader.loadFileCH1(file); // also persists to IndexedDB
        this._updateCH1Status();
        this._updateAnalyserLock();
        Persist.save();
    },
    ...
    async onCH2FileSelected(input) {
        ...
        await AudioLoader.loadFile(file); // also persists to IndexedDB
        this._updateCH2Status();

        const infoEl = document.getElementById('simFileInfo');
        if (infoEl) {
            infoEl.innerText   = 'loaded';
            infoEl.style.color = '#00ff41';
            infoEl.className   = 'sim-rec-timer';
        }
        Persist.save();
    },
    ...
    _updateAnalyserLock() {
        ...
        if (!unlocked) {
            App._analyseInterval = Infinity;
        } else if (!isFinite(App._analyseInterval)) {
            App._analyseInterval = 2000;
        }
    },
    ...
    toggleCH2Source() {}, // CH2 no longer reads mic — noop
