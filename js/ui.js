// ── UI.JS v1.3.2 ── channel color setters added
const UI = {
    ...
    _enterSimMode() {
        ...
        const oscPills = document.getElementById('oscPills');
        if (oscPills) oscPills.classList.toggle('visible', State.currentTab !== 'sim');
    ...
    _exitSimMode() {
        State.simMode = false; State.sim.playing = false;
        App.stopLoop();
        const oscPills = document.getElementById('oscPills');
    ...
    async runBodeSweep() {
        ...
        try {
            const result = await App.runBodeSweep(State.bode.startHz, State.bode.stopHz, State.bode.steps);
            if (!result || result.error) { if (btn) btn.innerText = '⚡ RUN SWEEP — SHOW ON SCOPE'; alert('Sweep failed — ensure signal is playing.'); return; }
            this._drawBodePanelCurves(result);
            const resGroup = document.getElementById('bodeResultsGroup'); if (resGroup) resGroup.style.display = 'block';
            this._populateBodeResults(result.features || {});
            this._drawBodeZone(result);
            if (btn) btn.innerText = '⚡ RUN SWEEP — SHOW ON SCOPE';
        } catch (err) { console.error('Bode sweep error:', err); if (btn) btn.innerText = '⚡ RUN SWEEP — SHOW ON SCOPE'; }
    },
    ...
    _drawBodePanelCurves(result) {
        ...
        const maxDb = Math.max(...dbs, 3), minDb = Math.min(...dbs, -40), dbRange = maxDb - minDb || 1;
        let magD = '', phaseD = '';
        curve.forEach((p, i) => {
    ...
    _drawBodeZone(result) {
        ...
        if (zone) { const wasVisible = zone.classList.contains('show'); zone.classList.toggle('show'); if (bodeBtn) bodeBtn.innerText = wasVisible ? '⚡ RUN SWEEP — SHOW ON SCOPE' : '⏹ HIDE BODE — RUN AGAIN TO TOGGLE'; }
        App.resize(); this.toggleSimSetMenu();
    },
    ...
    _syncPlayPauseButtons() {
        ...
        const dot = document.getElementById('statusDot'); const txt = document.getElementById('statusTxt');
    ...
    setTheme(theme) {
        ...
        if (theme === 'github') {
            if (!State._darkWaveColor) { State._darkWaveColor = State.waveColor; State._darkGridColor = State.gridColor; State._darkScopeText = State.scopeTextColor; State._darkMeasColor = State.measColor; }
            State.waveColor = '#cf222e'; State.gridColor = '#eaeef2'; State.scopeTextColor = '#0969da'; State.measColor = '#0969da';
        } else {
            if (State._darkWaveColor) { State.waveColor = State._darkWaveColor; State.gridColor = State._darkGridColor; State.scopeTextColor = State._darkScopeText; State.measColor = State._darkMeasColor; State._darkWaveColor = null; }
        }
        Persist.scheduleSave();
    },
    ...
    setCH1Color(color) {
        ...
        // In live mode ch1Color is also the waveColor
        State.waveColor = color;
        this._swatch('ch1-color', color);
        Persist.scheduleSave();
    },
    setCH2Color(color) { State.ch2Color = color; this._swatch('ch2-color', color); Persist.scheduleSave(); },
    setCH3Color(color) { State.ch3Color = color; this._swatch('ch3-color', color); Persist.scheduleSave(); },

    setGridColor(color) { State.gridColor = color; this._swatch('grid', color); Persist.scheduleSave(); },
    setMeasColor(color) { State.measColor = color; document.documentElement.style.setProperty('--amber', color); this._swatch('meas', color); Persist.scheduleSave(); },

    // kept for persist compat (no longer in UI but still called on restore)
    setWaveColor(color)          { State.waveColor = color; },
    setSettingsTextColor(color)  { State.settingsTextColor = color; document.documentElement.style.setProperty('--text-mid', color); document.documentElement.style.setProperty('--text-main', color); },
    setSettingsFont(font)        { State.settingsFont = font; const m = document.getElementById('sideMenu'); if (m) m.style.fontFamily = FONT_MAP[font] || FONT_MAP.retro; },
    setSettingsFontSize(v)       { State.settingsFontSize = parseInt(v); document.documentElement.style.setProperty('--settings-fs', (15 * State.settingsFontSize / 100) + 'px'); },
    setScopeTextColor(color)     { State.scopeTextColor = color; document.documentElement.style.setProperty('--neon', color); document.documentElement.style.setProperty('--neon-glow', color + '55'); },
    setScopeFont(font)           { State.scopeFont = font; document.documentElement.style.setProperty('--font-display', FONT_MAP[font] || FONT_MAP.retro); document.documentElement.style.setProperty('--font-scope', FONT_MAP[font] || FONT_MAP.retro); },
    setScopeFontSize(v)          { State.scopeFontSize = parseInt(v); document.documentElement.style.setProperty('--scope-fs', (15 * State.scopeFontSize / 100) + 'px'); },
    setInfoTextColor(color)      { State.infoTextColor = color; },
    setInfoFont(font)            { State.infoFont = font; document.documentElement.style.setProperty('--font-data', FONT_MAP[font] || FONT_MAP.mono); },
    setInfoFontSize(v)           { State.infoFontSize = parseInt(v); },

    _swatch(group, value) { document.querySelectorAll(`.swatch-btn[data-group="${group}"]`).forEach(b => b.classList.toggle('active', b.dataset.color === value)); },
    _font(group, value)   { document.querySelectorAll(`.font-pick-btn[data-group="${group}"]`).forEach(b => b.classList.toggle('active', b.dataset.font === value)); },
};
