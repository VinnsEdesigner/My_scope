...
const UI = {
    ...
    switchTab(tab) {
        ...
        // Hide HTML overlays on OSC + FFT — canvas draws same text there, causes clutter
        const hideOverlay = (tab === 'osc' || tab === 'fft');
        const oL = document.getElementById('overlayLeft');
        const oR = document.getElementById('overlayRight');
        if (oL) oL.style.display = hideOverlay ? 'none' : '';
        if (oR) oR.style.display = hideOverlay ? 'none' : '';
        ...
        try {
            for (let s = 0; s < steps; s++) {
                ...
                if (point) {
                    ...
                    // ── Also update the bode ZONE on main canvas live ──
                    const zw = 340, zh = 70;
                    let zMagD = '', zPhaseD = '';
                    for (let i = 0; i < curve.length; i++) {
                        const p   = curve[i];
                        const zpx = (i / (steps - 1)) * zw;
                        const zpy = zh - ((p.magnitudeDb - minDb) / dbRange) * (zh - 4) - 2;
                        const zpp = (zh / 2) - (p.phaseShiftDeg / 180) * (zh / 2 - 4);
                        zMagD   += (i === 0 ? 'M' : 'L') + zpx.toFixed(1) + ',' + Math.max(1, Math.min(zh - 1, zpy)).toFixed(1);
                        zPhaseD += (i === 0 ? 'M' : 'L') + zpx.toFixed(1) + ',' + Math.max(1, Math.min(zh - 1, zpp)).toFixed(1);
                    }
                    const zoneMag   = document.getElementById('bodeZoneMag');
                    const zonePhase = document.getElementById('bodeZonePhase');
                    const zone      = document.getElementById('bodeZone');
                    if (zoneMag)   zoneMag.setAttribute('d', zMagD);
                    if (zonePhase) zonePhase.setAttribute('d', zPhaseD);
                    if (zone && !zone.classList.contains('show')) zone.classList.add('show');
                    App.resize();
                }

                // ── YIELD + pace: ~100ms per step → 50 steps ≈ 5 sec sweep ──
    ...
    _drawBodePanelCurves(result) {
        ...
        const w = 340, h = 160, curve = result.curve;
