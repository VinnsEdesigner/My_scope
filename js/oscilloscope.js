// ── OSCILLOSCOPE.JS — time domain waveform renderer ──
const Oscilloscope = {
    draw(ctx, w, h) {
        if (w < 10 || h < 10) return;
        Grid.draw(ctx, w, h);
        ...
        // if trigger not fired in normal/single mode, skip waveform draw
        if (!fired && State.triggerMode !== 'auto') {
            // show "WAITING FOR TRIGGER" text
            ctx.fillStyle = 'rgba(255,179,0,0.6)';
            ctx.font      = "12px 'Share Tech Mono'";
            ctx.fillText('WAITING FOR TRIGGER...', w/2 - 90, h/2);
        }

        // draw threshold lines regardless
        Trigger.drawThresholds(ctx, w, h);

        if (!fired && State.triggerMode === 'normal') return;
