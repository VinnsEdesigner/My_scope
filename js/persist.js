// ── PERSIST.JS v1.0.3 — removed CSS var overrides from syncUIAfterRestore ──
...
const Persist = {
    ...
    _openDB() {
        return new Promise((resolve) => {
            ...
            req.onerror   = () => { console.warn('Persist: IndexedDB open failed'); resolve(null); };
        });
    },
    ...
    hasSession() { return !!localStorage.getItem('labscope_session'); },
    ...
    restoreFromLocalStorage() {
        try {
            ...
            if (d.timeDivIndex != null) s.timeDivIndex = Math.max(0, Math.min(TIME_DIV_STEPS.length - 1, d.timeDivIndex));
            if (d.voltDivIndex != null) s.voltDivIndex = Math.max(0, Math.min(VOLT_DIV_STEPS.length - 1, d.voltDivIndex));

            if (d.waveColor)  s.waveColor  = d.waveColor;
            if (d.ch1Color)   s.ch1Color   = d.ch1Color;
            if (d.ch2Color)   s.ch2Color   = d.ch2Color;
            if (d.ch3Color)   s.ch3Color   = d.ch3Color;
            if (d.gridColor)  s.gridColor  = d.gridColor;
            if (d.measColor)  s.measColor  = d.measColor;
            if (d.theme)      s.theme      = d.theme;

            // ── NOTE: intentionally NOT restoring settingsTextColor/Font/FontSize ──
            // Those override --text-mid/--text-main and break grey UI text
            ...
            s.sim.playing = false; // never restore playing
    ...
    async saveAnalyserResult(result) {
        if (!result) return;
        if (!this._db) await this._openDB();
        try { await this._idbSet('analyserResult', JSON.stringify(result)); }
        catch(e) { console.warn('Persist.saveAnalyserResult failed:', e); }
    },

    async clearCH1Buffer() { await this._idbDelete('ch1PcmBuffer'); localStorage.removeItem('labscope_ch1FileName'); },
    async clearCH2Buffer() { await this._idbDelete('ch2PcmBuffer'); localStorage.removeItem('labscope_ch2FileName'); },
    ...
    async restoreFromIndexedDB() {
        ...
        try {
            ...
            if (arRaw && typeof arRaw === 'string') {
                try { State.analyserResult = JSON.parse(arRaw); }
                catch(e) { State.analyserResult = null; }
            }
        } catch(e) { console.warn('Persist.restoreFromIndexedDB failed:', e); }
    },
    ...
    scheduleSave() {
        ...
        this._saveTimer = setTimeout(() => { this.saveToLocalStorage(); this._saveTimer = null; }, this._DEBOUNCE);
    },
    ...
    async checkMicPermission() {
        try {
            if (!navigator.permissions) return this.hasSession();
            const result = await navigator.permissions.query({ name: 'microphone' });
            return result.state === 'granted';
        } catch(e) { return this.hasSession(); }
    },
    ...
    syncUIAfterRestore() {
        try {
            ...
            this._setText('trigUpperVal', (s.triggerUpper >= 0 ? '+' : '') + s.triggerUpper.toFixed(2) + 'V');
            this._setText('trigLowerVal', (s.triggerLower >= 0 ? '+' : '') + s.triggerLower.toFixed(2) + 'V');
            ...
            this._setText('bodeSweepStart', s.bode.startHz < 1000 ? s.bode.startHz + 'Hz' : (s.bode.startHz / 1000).toFixed(0) + 'kHz');
            this._setText('bodeSweepStop',  s.bode.stopHz  < 1000 ? s.bode.stopHz  + 'Hz' : (s.bode.stopHz  / 1000).toFixed(0) + 'kHz');

            const freqLabel = s.sim.frequency >= 1000 ? (s.sim.frequency / 1000).toFixed(2) + 'kHz' : s.sim.frequency + 'Hz';
            ...
            if (s.ch1Color) UI._swatch('ch1', s.ch1Color);
            if (s.ch2Color) UI._swatch('ch2', s.ch2Color);
            if (s.ch3Color) UI._swatch('ch3', s.ch3Color);
            if (s.measColor) {
                UI._swatch('meas', s.measColor);
                document.documentElement.style.setProperty('--amber', s.measColor);
            }

            // ── NOTE: NOT calling setSettingsTextColor / setSettingsFont / setSettingsFontSize ──
            // Those override --text-mid/--text-main which makes ALL UI text cyan.
            // Scope neon color only (canvas overlay, doesn't affect UI text)
            if (s.scopeTextColor) {
                document.documentElement.style.setProperty('--neon', s.scopeTextColor);
            }

            // Fonts (safe — don't affect text color)
    ...
    _setText(id, val) { const el = document.getElementById(id); if (el && val != null) el.innerText = val; },
    _setSlider(id, val) { const el = document.getElementById(id); if (el && val != null) el.value = val; },
    ...
    _updateFilenameDisplay(ch, name) {
        if (ch === 'ch1') {
            ...
            ['simFileName','ch2FileName'].forEach(id => {
                ...
                if (el) { el.innerText = name; el.classList.remove('empty'); el.style.color = '#00e5ff'; }
            });
        }
    },
};
